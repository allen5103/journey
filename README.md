...
[開啟公開版本](https://allen5103.github.io/journey/) · [查看最新版本](https://allen5103.github.io/journey/?v=1.8.1)

目前公開版本：**v1.8.1**。只有裝置今天落於旅程起訖日期內時，開啟旅程才會依當下時間切換至對應日期並精確捲動至進行中或下一個行程；不在旅程日期範圍時會保留原本選取的日期與畫面位置。底部「旅程、今天、設定」導覽使用自製地圖、日曆與調整滑桿 SVG 純線條圖示，三者統一為 20px 尺寸、1.8px 筆畫並保留文字標籤及選取色彩。新增與編輯行程表單提供「清除 Google Maps 資訊」操作，會清除地圖搜尋、Place ID 與座標並即時移除地圖預覽；純文字地點與其他行程資料會保留，按「儲存變更」後才寫入。單一行程卡片會在同一右側直欄由上而下顯示行程類別、編輯與刪除按鈕，三個控制項統一為 30px 高度及 4px 固定間距；類別、時間、完成與目前行程的背景會改用所屬旅程選取色彩。完成勾選框與「完成數／總數」文字目前暫時隱藏，但既有完成資料不會被刪除。左側資訊會垂直置中，含預覽圖片時縮圖撐滿資訊區塊高度。旅程詳情頁的返回按鈕靠左，匯出、編輯與刪除按鈕靠右，四者會在向下捲動時固定於頂端的同一條 sticky 橫排；設定頁版本號旁提供自製 SVG 重新整理按鈕。首次在線開啟後，公開網站會快取應用程式殼層；離線時左下角顯示「目前離線中」，並隱藏 Google Maps、地點搜尋與 Google Maps 外連介面。請在應用程式的「設定」頁查看實際載入的版本號。
...
當同一天有多筆行程時，工具會依開始時間排序，並在相鄰行程之間計算空檔時間。進入旅程詳情時，僅在裝置今天位於該旅程的起訖日期內，系統才會依裝置時間標示「現在進行中」或「下一個行程」，並定位到對應日期；旅程期間外則維持原本選取日期與列表位置。
...
行程表單採原生日期與時間選擇器。**行程類型**可選擇景點、餐飲、交通、住宿、購物或其他；純文字地點與 Google Maps 搜尋欄位分開保存，因此選擇地圖建議不會覆寫自行輸入的地點文字。選取照片後，編輯頁會立即顯示最多三張待保存預覽，無須先儲存；每張待保存縮圖都有「移除這次選取」按鈕，可單獨取消某張照片。此時預覽只存在目前表單，按下儲存後才會寫入 IndexedDB 與旅程資料。

![行程編輯：日期、時間、類型、地點與地圖搜尋](docs/images/02-activity-editor.webp)

### 3. 三張照片與全螢幕檢視

單一行程最多可保存三張照片。選取新照片後會先立即顯示待保存縮圖；待保存縮圖可逐張移除，不會影響已保存照片；編輯頁也會並排顯示全部已保存縮圖；點擊縮圖可開啟深色全螢幕檢視器，使用左右按鈕切換照片。行程卡片只顯示第一張縮圖，使每日清單維持簡潔。每張照片右上角的 **×** 會在確認後立即移除該照片。

![照片全螢幕檢視器與前後切換控制](docs/images/03-photo-gallery-viewer.webp)

### 4. 設定與儲存維護

設定頁顯示網頁版本，版本號旁可按自製 SVG 重新整理按鈕重新載入頁面；同時顯示照片資料庫用量、網站總用量與瀏覽器提供的可用空間估算。若先前操作留下未被任何行程引用的照片，可使用「清理未使用圖片」安全釋放空間。
...
在公開網站版本，行程表單可使用 Google Maps Places 自動完成搜尋地點。從建議中選取項目後，系統保存座標與地圖搜尋文字，並在行程詳情顯示地圖預覽及「在 Google Maps 開啟」連結。若想改用純文字地點或取消已選地點，可按表單內的「清除 Google Maps 資訊」；此操作只清除地圖搜尋、Place ID 與座標，立即移除預覽，並保留純文字地點。按「儲存變更」後才會寫回行程。
...
**匯出單一旅程：** 在旅程詳情頁頂端 sticky 操作列按「匯出」。
